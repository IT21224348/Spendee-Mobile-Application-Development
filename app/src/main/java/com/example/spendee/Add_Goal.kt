...
class Add_Goal : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.AddLoan.setOnClickListener {
           if(checkAllfields()){
               ...
               val goalname= binding.loanName.text.toString()
               val targetdate = binding.loanAmount.text.toString()
               val goalamount = binding.loanDate.text.toString().toDouble()
               val balanceamount = binding.loanSettle.text.toString().toDouble()

               database = FirebaseDatabase.getInstance().getReference("Goals")
               val goals = Goal_model(goalname,targetdate,goalamount,balanceamount)

               if (userId != null){

                   database.child(userId).push().setValue(goals).addOnSuccessListener {
                       ...
                       Toast.makeText(this, "Goal added successfully", Toast.LENGTH_SHORT)
                           .show()
        ...
        registerForActivityResult<Intent, androidx.activity.result.ActivityResult>(
            ...
            if (result.resultCode == 2) {
                ...
                if (intent != null) {
                    ...
                    nodeId = intent.getStringExtra("goal_id").toString()
                }
                if (userId != null) {
                    database = FirebaseDatabase.getInstance().getReference("Goals")
                    ...
                    database.child(userId.toString()).child(nodeId).get().addOnSuccessListener {
                        if (it.exists()) {
                            ...
                            binding.loanAmount.setText(it.child("date").value.toString())
                            binding.loanDate.setText(it.child("target").value.toString())
                            binding.loanSettle.setText(it.child("balance").value.toString())
    ...
    fun Update_date(view: View) {
        ...
        val targetdate = binding.loanAmount.text.toString()
        val targetamount = binding.loanDate.text.toString().toDouble()
        val balancedamount = binding.loanSettle.text.toString().toDouble()

        database = FirebaseDatabase.getInstance().getReference("Goals")
        val borrowed_loan =
            Goal_model(Target, targetdate, targetamount, balancedamount)
        val userId = FirebaseAuth.getInstance().currentUser?.uid
        ...
        fun Delete_Data(view: View) {
            ...
            if (userId != null) {
                database.child(userId).child(nodeId).removeValue().addOnSuccessListener {
                    ...
                    Toast.makeText(this, "Target Deleted", Toast.LENGTH_SHORT).show()
                }.addOnFailureListener {
